<?php

namespace App\Tests\Controller;

use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;
use GuzzleHttp\Client as Client;

class GroupsControllerTest extends WebTestCase
{
    public function setUp()
    {
        $this->http = new Client(['base_uri' => '127.0.0.1:8000/v1/']);

        $this->setToken();
    }

    public function test_group_list()
    {
        $response = $this->http->request('GET', 'groups', ['headers' => ['X-AUTH-TOKEN' => $this->token]]);

        $this->assertEquals(200, $response->getStatusCode());

        $contentType = $response->getHeaders()["Content-Type"][0];
        $this->assertEquals("application/json", $contentType);
    }

    public function test_group_create()
    {
        $numberRnd = rand(0, 9999);

        $response = $this->http->request('POST', 'groups', ['headers' => ['X-AUTH-TOKEN' => $this->token],'query' => ['name' => 'TestGrpName_'.$numberRnd,'description' => 'TestGrpDescription_'.$numberRnd]]);

        $this->assertEquals(201, $response->getStatusCode());
    }

    public function test_group_update()
    {
        $numberRnd = rand(0, 9999);

        $response = $this->http->request('POST', 'groups', ['headers' => ['X-AUTH-TOKEN' => $this->token],'query' => ['name' => 'TestGrpName_'.$numberRnd,'description' => 'TestGrpDescription_'.$numberRnd]]);

        $body = $response->getBody();
        $data = json_decode($body->getContents(), true);

        $response = $this->http->request('PUT', 'groups/'.$data['id'], ['headers' => ['X-AUTH-TOKEN' => $this->token],'query' => ['name' => 'TestGrpNameMod_'.time(),'description' => 'TestGrpDescriptionMod_'.time()]]);

        $this->assertEquals(200, $response->getStatusCode());
    }


    public function test_group_delete()
    {
        $numberRnd = rand(0, 9999);

        $response = $this->http->request('POST', 'groups', ['headers' => ['X-AUTH-TOKEN' => $this->token],'query' => ['name' => 'TestGrpName_'.$numberRnd,'description' => 'TestGrpDescription_'.$numberRnd]]);

        $body = $response->getBody();
        $data = json_decode($body->getContents(), true);

        $response = $this->http->request('DELETE', 'groups/'.$data['id'], ['headers' => ['X-AUTH-TOKEN' => $this->token]]);

        $this->assertEquals(200, $response->getStatusCode());
    }

    public function test_group_search()
    {
        $response = $this->http->request('GET', 'groups/search', ['headers' => ['X-AUTH-TOKEN' => $this->token],'param' => '{name:\'TestGrpName_\'}','order' =>'{name:\'asc\'}']);

        $this->assertEquals(200, $response->getStatusCode());
    }


    private function setToken()
    {
        $response = $this->http->request('GET', 'login', ['query' => ['username' => 'internations@gmail.com','password' => 'hireme']]);

        $body = $response->getBody();
        $data = json_decode($body->getContents(), true);

        $this->token = $data['body'];
    }
}
